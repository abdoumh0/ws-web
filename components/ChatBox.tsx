      ...
      <div className={`${chat.ChatBox == "OPEN" ? "" : "hidden"}`}>
          ...
          {chat.Messages.length > 0 ? (
            chat.Messages.toReversed().map((message) => {
              return (
                <div
                  className={`${
                    message.SenderUsername == session?.Username
                      ? "text-right  ml-auto bg-indigo-600 text-white"
                      : " bg-gray-100"
                  } w-fit px-3 py-2.5 rounded-4xl my-1 text-sm max-w-48 break-words`}
                  key={message.MessageID}
                >
                  {message.MessageContent.at(0)?.Text}
                </div>
              );
            })
          ) : (
            <div className="w-full h-full text-sm flex items-center justify-center text-gray-600">
              No messages.
            </div>
          )}
        </div>
        <div className="flex justify-between border-border border-t-2 py-1 px-2 gap-x-4">
          <textarea
            ...
            name=""
            id=""
            className="outline-none resize-none border border-border rounded-2xl bg-indigo-50 p-2 text-sm"
          />
          <button
            onClick={() => {
              if (!textareaRef.current) return;
              if (textareaRef.current.value.trim() === "") return;
              const sent = handleMessageSend(ChatID, textareaRef.current.value);
              if (sent) textareaRef.current.value = "";
            }}
            className="text-center rounded-full bg-indigo-500 flex justify-center items-center text-white w-[40px] h-[40px] hover:brightness-105 active:brightness-95"
          >
            <Send size={20} />
          </button>
        </div>
