                  ...
                  onClick={async () => {
                    const chat = ChatStore.find((c) => {
                      ...
                      return (
                        type == "DM" &&
                        members.length == 2 &&
                        members.includes(session.Username!) &&
                    ...
                    if (chat) {
                      ...
                      ChatStoreDispatch({
                        type: "ADD",
                        chat: { ...mockchat, ChatBox: "OPEN" },
                      });
                      try {
                        setCreatingChat(true);
                        let res = await fetch(
                          `/api/chats/get_dm?target=${Username}`
                        );
                        const dm = (await res.json()) as {
                          ok: boolean;
                          DM: ChatType;
                        };
                        if (dm.ok && dm.DM) {
                        ...
                        const { ok, newChat } = (await res.json()) as {
                          ok: boolean;
                          newChat: ChatType;
                        };
                        if (ok && newChat) {
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="mb-6">
          ...
          <p className="text-slate-600">
            Browse items from this seller's inventory
          </p>
        </div>
              ...
              <div className="mb-6">
                ...
                <div className="space-y-3">
                  {mockUserItems.map((item) => (
                    <div
                      key={item.ItemID}
                      className="flex items-center space-x-4 p-4 border border-slate-200 rounded-lg hover:bg-slate-50"
                    >
                      <input
                        type="checkbox"
